...
Initial corpus (≥5 PDFs): simple table, nested table, page-spanning table,
multi-column page, document with sections. Grow as real cases arrive.

Adding a case: run the parser, hand-inspect the rendered HTML (also written by
the script), commit the PDF + JSON. Diff-driven workflow — when a change
shifts a tree, the diff shows exactly what regressed.
